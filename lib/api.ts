...
export async function createPot(input: {
  ...
  const now = Date.now()
  const deadlineAt = new Date(now + input.deadlineMinutes * 60_000).toISOString()
  const pickupAt = new Date(now + input.pickupMinutes * 60_000).toISOString()
  ...
  const newPot: Pot = {
    ...
    currentCount: 1,
    currentAmount: input.targetType === 'AMOUNT' ? 0 : undefined,
