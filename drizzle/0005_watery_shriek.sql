...
CREATE TABLE "manner_profiles" (
	...
	CONSTRAINT "manner_profiles_score_check" CHECK ("manner_profiles"."score" >= 0 AND "manner_profiles"."score" <= 100)
);
--> statement-breakpoint
CREATE TABLE "manner_reviews" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "manner_reviews_no_self_review_check" CHECK ("manner_reviews"."reviewer_id" <> "manner_reviews"."reviewee_id")
);
--> statement-breakpoint
ALTER TABLE "pots" ADD COLUMN "ordered_at" timestamp with time zone;--> statement-breakpoint
ALTER TABLE "manner_events" ADD CONSTRAINT "manner_events_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "manner_events" ADD CONSTRAINT "manner_events_review_id_manner_reviews_id_fk" FOREIGN KEY ("review_id") REFERENCES "public"."manner_reviews"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "idx_manner_reviews_reviewee" ON "manner_reviews" USING btree ("reviewee_id","applied_at");